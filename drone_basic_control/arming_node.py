...
class ArmingNode(LifecycleNode):
    def __init__(self, drone_ns: str = "drone_1", px4_ns: str = "px4_1", px4_sys_id: int = 1):
        ...
        self.get_logger().info(f'[{self.drone_ns}] ArmingNode init for PX4 ns: {self.px4_ns} (sys_id={self.px4_sys_id})')
    ...
    def compose_vehicle_command(self, command, **params) -> None:
        ...
        msg.target_system = self.px4_sys_id  # <-- critical for multi-drone
    ...
    def timer_callback(self):
        # First ensure OFFBOARD mode (heartbeat + setpoints must already be flowing from hb_node)
        if self.vehicle_status.nav_state != VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            self.get_logger().info(f'[{self.drone_ns}] Switching to OFFBOARD...')
