...
from drone_msgs.action import Land


class LandDrone(py_trees.behaviour.Behaviour):
    def __init__(self, node, drone_ns: str = "drone_1"):
        super().__init__(name=f"LandDrone_{drone_ns}")
        self.node = node
        self.drone_ns = drone_ns
        self.client = ActionClient(node, Land, f'/{self.drone_ns}/land')
    ...
    def update(self):
        ...
        if self._result_future is not None and self._result_future.done():
            ...
            self._done = True
            return py_trees.common.Status.SUCCESS if result.landing_status else py_trees.common.Status.FAILURE
